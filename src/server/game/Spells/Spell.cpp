...
Spell::~Spell()
{
    ...
    if (m_caster && m_caster->GetTypeId() == TYPEID_PLAYER)
        ...
        {
            TC_LOG_FATAL("spells", "SPELL: deleting spell for spell ID {}. However, spell still referenced. in m_spellModTakingSpell", m_spellInfo->Id);
...
SpellCastResult Spell::prepare(SpellCastTargets const& targets, AuraEffect const* triggeredByAura)
{
    ...
    if (m_caster->IsPlayer() && m_caster->ToUnit()->isMoving())
    {
        ...
        {
            SendCastResult(result);
            finish(result); ///< Needed so door of shadow isnt instant
            return result;
        }
    }
    ...
    if (Unit* target = m_targets.GetUnitTarget())
    {
        ...
        if (target != m_caster)
        {
            ...
            if (m_caster->GetTypeId() != TYPEID_GAMEOBJECT)
            {
                ...
                if (!m_spellInfo->HasAttribute(SPELL_ATTR2_IGNORE_LINE_OF_SIGHT) && !DisableMgr::IsDisabledFor(DISABLE_TYPE_SPELL, m_spellInfo->Id, nullptr, SPELL_DISABLE_LOS) && !losTarget->IsWithinLOSInMap(target, LINEOFSIGHT_ALL_CHECKS, VMAP::ModelIgnoreFlags::M2))
        ...
        switch (spellEffectInfo.Effect)
        {
            ...
            case SPELL_EFFECT_CHARGE:
            {
                ...
                if (GetSpellInfo()->NeedsExplicitUnitTarget())
                {
                    ...
                    m_preGeneratedPath = std::make_unique<PathGenerator>(unitCaster, hasTransport);
            ...
            case SPELL_EFFECT_SKINNING:
            {
                ...
                int32 ReqValue = 1; // this is 1 since shadowlands. higherlevel skinning = better loot NYI //(skillValue < 100 ? (TargetLevel-10) * 10 : TargetLevel * 5);
    ...
    switch (spellEffectInfo.Effect)
    {
        ...
        default:
        {
            if (!losPosition || m_spellInfo->HasAttribute(SPELL_ATTR5_ALWAYS_AOE_LINE_OF_SIGHT))
            {
                ...
                if (!caster)
                    caster = m_caster;
                if (target != m_caster && !caster->IsWithinLOSInMap(target, LINEOFSIGHT_ALL_CHECKS, VMAP::ModelIgnoreFlags::M2))
                    return false;
            }
...
void Spell::TriggerGlobalCooldown()
{
    ...
    if (gcd >= MinGCD && gcd <= MaxGCD)
    {
        ...
        // Apply haste rating
       // if (gcd > MinGCD && ((m_spellInfo->StartRecoveryCategory == 133 && !isMeleeOrRangedSpell) || m_caster->ToUnit()->HasAuraTypeWithAffectMask(SPELL_AURA_MOD_GLOBAL_COOLDOWN_BY_HASTE, m_spellInfo)))
        {
