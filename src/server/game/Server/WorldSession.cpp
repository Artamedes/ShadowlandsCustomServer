...
WorldSession::WorldSession(uint32 id, std::string&& name, uint32 battlenetAccountId, std::shared_ptr<WorldSocket> sock, AccountTypes sec, uint8 expansion, time_t mute_time,
    ...
    m_accountExpansion(EXPANSION_DRAGONFLIGHT),
    m_expansion(EXPANSION_DRAGONFLIGHT),
    _os(os),
...
bool WorldSession::Update(uint32 diff, PacketFilter& updater)
{
    ...
    if (IsConnectionIdle() && !HasPermission(rbac::RBAC_PERM_IGNORE_IDLE_CONNECTION) && !m_IsStabilityTestSession)
    ...
    if (updater.ProcessUnsafe() && !IsStabilityTest())
    {
...
uint32 WorldSession::DosProtection::GetMaxPacketCounterAllowed(uint16 opcode) const
{
    ...
    switch (opcode)
    {
        ...
        case CMSG_MOVE_SET_PITCH: // disabled
        case CMSG_REQUEST_MYTHIC_PLUS_AFFIXES:
        {
