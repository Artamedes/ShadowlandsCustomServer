    ...
    switch (e.GetActionType())
    {
        ...
        case SMART_ACTION_CALL_KILLEDMONSTER:
        {
            if (e.target.type == SMART_TARGET_NONE || e.target.type == SMART_TARGET_SELF) // Loot recipient and his group members
            {
                ...
                for (ObjectGuid tapperGuid : me->GetTapList())
                {
                    ...
                    {
                        tapper->KilledMonsterCredit(e.action.questCredit.creditId, me->GetGUID());
                        TC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_CALL_KILLEDMONSTER: Player %s, Killcredit: %u",
                            tapper->GetGUID().ToString().c_str(), e.action.questCredit.creditId);
                    }
                }
            }
        ...
        case SMART_ACTION_MOVE_TO_POS:
        {
            ...
            if (!target)
            {
                ...
                me->GetMotionMaster()->MovePoint(e.action.moveToPos.pointId, dest, e.action.moveToPos.disablePathfinding ? 0 : MOVE_PATHFINDING);
            }
            else
            {
                float x, y, z;
                ...
                me->GetMotionMaster()->MovePoint(e.action.moveToPos.pointId, x + e.target.x, y + e.target.y, z + e.target.z, e.action.moveToPos.disablePathfinding ? 0 : MOVE_PATHFINDING);
            }
            break;
        }
