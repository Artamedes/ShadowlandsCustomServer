...
Loot::Loot(Map* map, ObjectGuid owner, LootType type, Group const* group) : gold(0), unlootedCount(0), roundRobinPlayer(), loot_type(LOOT_NONE), maxDuplicates(1),
...
bool Loot::FillLoot(uint32 lootId, LootStore const& store, Player* lootOwner, bool personal, bool noEmptyError, uint16 lootMode /*= LOOT_MODE_DEFAULT*/, ItemContext context /*= ItemContext::NONE*/, bool specOnly /*= false*/, bool personalLoot /*= false*/, bool fishing /* = false*/, bool oploteChest /*= false*/)
{
    // Must be provided
    if (!lootOwner)
        return false;
...
void Loot::AddItem(LootStoreItem const& item, Player const* player, bool personalLoot /*= false*/, bool isOploteLoot /*= false*/)
{
    switch (item.type)
    {
