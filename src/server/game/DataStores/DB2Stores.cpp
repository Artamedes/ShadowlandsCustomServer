...
DB2Storage<UICovenantAbilityEntry>              sUiCovenantAbilityEntry("UICovenantAbility.db2", UiCovenantAbilityLoadInfo::Instance());
...
#define LOAD_DB2(store) LoadDB2(availableDb2Locales, loadErrors, _stores, &(store), db2Path, defaultLocale, GetCppRecordSize(store))
    ...
    LOAD_DB2(sUiCovenantAbilityEntry);
...
#undef LOAD_DB2
    ...
    for (auto entry : sGossipNPCOptionStore)
        _gossipNPCOptionsByIndex[entry->GossipOptionID] = entry;
