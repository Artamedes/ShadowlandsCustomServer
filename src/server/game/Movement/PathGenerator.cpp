...
bool PathGenerator::CalculatePath(float destX, float destY, float destZ, G3D::Vector3& start, bool forceDest, bool straightLine)
{
    ...
    if (_transport)
    {
        _transport->CalculatePassengerOffset(start.x, start.y, start.z);
...
bool PathGenerator::CalculatePath(Position startPos, Position destPos, bool forceDest, bool straightLine)
{
    ...
    {
        _transport->CalculatePassengerOffset(start.x, start.y, start.z);
        _transport->CalculatePassengerOffset(dest.x, dest.y, dest.z);
    }
...
void PathGenerator::NormalizePath()
{
    if (_transport)
    {
        ...
        {
            for (uint32 i = 0; i < _pathPoints.size(); ++i)
                _transport->CalculatePassengerPosition(_pathPoints[i].x, _pathPoints[i].y, _pathPoints[i].z);
        }
        return;
    }
