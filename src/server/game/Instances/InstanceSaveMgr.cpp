...
*/
InstanceSave* InstanceSaveManager::AddInstanceSave(uint32 mapId, uint32 instanceId, Difficulty difficulty, time_t resetTime, uint32 entranceId, uint32 completedEncounters, bool canReset, bool load)
{
    ...
    InstanceSave* save = new InstanceSave(mapId, instanceId, difficulty, entranceId, completedEncounters, resetTime, canReset);
    if (!load)
        save->SaveToDB();
...
InstanceSave::InstanceSave(uint16 MapId, uint32 InstanceId, Difficulty difficulty, uint32 entranceId, uint32 completedEncounters, time_t resetTime, bool canReset)
: m_resetTime(resetTime), m_instanceid(InstanceId), m_mapid(MapId),
  m_difficulty(difficulty), m_entranceId(entranceId), m_completedEncounter(completedEncounters), m_canReset(canReset), m_toDelete(false), m_extended(false) { }
