...
bool LoginQueryHolder::Initialize()
{
    ...
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_LOAD_ACC_ACHIEVEMENTS, stmt);
    ...
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CLAIMED_DAILY_REWARDS);
    stmt->setUInt64(0, lowGuid);
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_LOAD_CHARACTER_DAILY_REWARDS, stmt);
    ...
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_SPEC_INFO);
    stmt->setUInt64(0, lowGuid);
    res &= SetPreparedQuery(PLAYER_LOGIN_QUERY_LOAD_SEL_SPEC_INFO, stmt);
    ...
    bool checkRequiredDependentChoices, Trinity::IteratorPair<UF::ChrCustomizationChoice const*> selectedChoices) const
{
    ...
    if (req->ReqQuestID)
    {
        ...
        if (!_player->IsQuestRewarded(req->ReqQuestID))
            return false;
    }
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder const& holder)
{
    ...
    pCurrChar->GetMap()->AddTask([pCurrChar]()
    {
        ...
        //    pCurrChar->GetXP(), pCurrChar->GetXPForNextLevel(), std::max(0, (int32)pCurrChar->GetXPForNextLevel() - (int32)pCurrChar->GetXP()));
        ...
        if (pCurrChar->GetPet())
        {
            ...
            for (AuraEffect const* aurEff : animalCompanion)
                if (sSpellMgr->GetSpellInfo(aurEff->GetTriggerSpell()))
                    pCurrChar->CastSpell(pCurrChar, aurEff->GetTriggerSpell(), true);
        }
        ...
        pCurrChar->UpdateCriteria(CriteriaType::Login, 1);
