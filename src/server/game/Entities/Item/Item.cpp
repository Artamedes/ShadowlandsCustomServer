...
bool Item::LoadFromDB(ObjectGuid::LowType guid, ObjectGuid ownerGuid, Field* fields, uint32 entry)
{
    ...
    //                       49                      50    51
    //        fixedScalingLevel, artifactKnowledgeLevel, createTime FROM item_instance
  ...
  //  SetUpdateFieldValue(m_values.ModifyValue(&Item::m_itemData).ModifyValue(&UF::ItemData::CreateTime), fields[51].GetUInt32());
...
{
    if (checkExists && std::find(GetBonusListIDs().begin(), GetBonusListIDs().end(), int32(bonusListID)) != GetBonusListIDs().end())
        return false;
...
bool Item::RemoveBonus(uint32 bonusListID)
{
    ...
    bonusIds.reserve(GetBonusListIDs().size());
    for (auto bonus : GetBonusListIDs())
...
/*static*/ std::string Item::GetItemLink(Item* item, Player* player)
{
    ...
    switch (l_Proto->GetId())
    {
        ...
        default:
            ...
            l_SS << item->GetBonusListIDs().size() << ":"; // num bonusIds
            for (auto bonusId : item->GetBonusListIDs())
