...
/*static*/ uint32 Unit::DealDamage(Unit* attacker, Unit* victim, uint32& damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
    ...
    // call script hooks
    {
        ...
        if (UnitAI* attackerAI = attacker ? attacker->GetAI() : nullptr)
            attackerAI->DamageDealt(victim, tmpDamage, damagetype, spellProto);

        // Hook for OnDamage Event
        if (attacker && victim)
            sScriptMgr->OnDamage(attacker, victim, tmpDamage, spellProto);
    ...
    if (attacker && cleanDamage && (cleanDamage->attackType == BASE_ATTACK || cleanDamage->attackType == OFF_ATTACK) && damagetype == DIRECT_DAMAGE && attacker != victim && attacker->GetPowerType() == POWER_RAGE)
    {
        ...
        uint32 attTime = attacker->GetBaseAttackTime(cleanDamage->attackType);

        if (isPlayer && sWorld->getIntConfig(WorldIntConfigs::CONFIG_HASTE_CAP))
            attTime = 1000;
        uint32 rage = uint32(1000 / 1000.f * 1.75f);
...
/*static*/ void Unit::CalcHealAbsorb(HealInfo& healInfo)
{
    ...
    if (!vHealAbsorb.empty())
    {
        for (AuraEffectList::const_iterator i = vHealAbsorb.begin(); i != vHealAbsorb.end() && healInfo.GetHeal() > 0; ++i)
        {
            ...
            if (!(aura->GetMiscValue() & healInfo.GetSpellInfo()->SchoolMask))
                continue;
...
void Unit::UpdateUnitMod(UnitMods unitMod)
{
    ...
    switch (unitMod)
    {
        ...
        case UNIT_MOD_FURY:
        case UNIT_MOD_PAIN:
        case UNIT_MOD_ESSENCE:            UpdateMaxPower(Powers(unitMod - UNIT_MOD_POWER_START));     break;
...
/*static*/ void Unit::Kill(Unit* attacker, Unit* victim, bool durabilityLoss /*= true*/, bool skipSettingDeathState /*= false*/)
{
    ...
    if (!victim->IsCritter())
    {
        Unit::ProcSkillsAndAuras(attacker, victim, { PROC_FLAG_KILL, PROC_FLAG_2_TARGET_DIES }, PROC_FLAG_NONE, PROC_SPELL_TYPE_MASK_ALL, PROC_SPELL_PHASE_NONE, PROC_HIT_NONE, nullptr, nullptr, nullptr);
...
void Unit::SendLossOfControlAuraUpdate(AuraApplication const* aurApp, Mechanics mechanic, SpellEffIndex effIndex, LossOfControlType type)
{
    ...
    if (aura->GetSpellInfo()->HasAttribute(SPELL_ATTR0_DO_NOT_DISPLAY_SPELLBOOK_AURA_ICON_COMBAT_LOG) || aura->GetSpellInfo()->HasAttribute(SPELL_ATTR5_DO_NOT_DISPLAY_DURATION))
        return;
...
void Unit::SendAddLossOfControl(AuraApplication const* aurApp, Mechanics mechanic, LossOfControlType type)
{
    ...
    if (aura->GetSpellInfo()->HasAttribute(SPELL_ATTR0_DO_NOT_DISPLAY_SPELLBOOK_AURA_ICON_COMBAT_LOG) || aura->GetSpellInfo()->HasAttribute(SPELL_ATTR5_DO_NOT_DISPLAY_DURATION) || aura->GetSpellInfo()->HasAttribute(SPELL_ATTR1_NO_AURA_ICON))
        return;
