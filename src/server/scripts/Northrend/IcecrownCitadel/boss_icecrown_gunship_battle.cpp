...
public:
    ...
    bool Execute(uint64, uint32) override
    {
        ...
        if (Transport* go = HashMapHolder<MapTransport>::Find(_transport))
            go->AddObjectToRemoveList();

        if (Transport* go = HashMapHolder<MapTransport>::Find(_otherTransport))
            go->AddObjectToRemoveList();
        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_ADDS:
                    ...
                    if (Transport* skybreaker = HashMapHolder<MapTransport>::Find(_instance->GetGuidData(DATA_ICECROWN_GUNSHIP_BATTLE)))
        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_ADDS:
                    ...
                    if (Transport* orgrimsHammer = HashMapHolder<MapTransport>::Find(_instance->GetGuidData(DATA_ICECROWN_GUNSHIP_BATTLE)))
...
struct npc_gunship_boarding_addAI : public gunship_npc_AI
{
    ...
    void MovementInform(uint32 type, uint32 pointId) override
    {
        ...
        if (pointId == EVENT_CHARGE_PREPATH && Slot)
        {
            ...
            if (Transport* destTransport = HashMapHolder<MapTransport>::Find(Instance->GetGuidData(DATA_ICECROWN_GUNSHIP_BATTLE)))
...
class spell_igb_gunship_fall_teleport : public SpellScript
{
    ...
    void SelectTransport(WorldObject*& target)
    {
        if (InstanceScript* instance = target->GetInstanceScript())
            target = HashMapHolder<MapTransport>::Find(instance->GetGuidData(DATA_ICECROWN_GUNSHIP_BATTLE));
    }
