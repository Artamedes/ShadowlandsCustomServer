...
class instance_deadmines : public InstanceMapScript
{
    public:
        ...
        struct instance_deadmines_InstanceMapScript : public InstanceScript
        {
            ...
            void OnCreatureCreate(Creature* creature) override
            {
                ...
                switch (creature->GetEntry())
                {
                    ...
                    case NPC_DEFIAS_SHADOWGUARD:
                        ...
                        break;
                    case NPC_ROPE:
                        creature->SetNpcFlag(UNIT_NPC_FLAG_SPELLCLICK);
            ...
            bool SetBossState(uint32 type, EncounterState state, bool forced) override
            {
                if (!InstanceScript::SetBossState(type, state, forced))
                    return false;
            ...
            void ResetVanessasNightmare()
            {
                ...
                if (Creature* trapBunny = GetCreature(DATA_VANESSAS_TRAP_BUNNY))
                    trapBunny->DespawnOrUnsummon(0s, Seconds(30));

                if (Creature* anchorBunny = GetCreature(DATA_VANESSA_ANCHOR_BUNNY))
                    anchorBunny->DespawnOrUnsummon(0s, Seconds(30));
                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_SUMMON_GLUBTOK:
                            if (Creature* glubtok = instance->SummonCreature(NPC_GLUBTOK_NIGHTMARE, GlubtokNightmareSpawnPos))
                                glubtok->RemoveUnitFlag(UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_IMMUNE_TO_PC);
                            break;
                        ...
                        case EVENT_SUMMON_FOE_REAPER:
                            ...
                            {
                                reaper->RemoveUnitFlag(UNIT_FLAG_IMMUNE_TO_NPC | UNIT_FLAG_IMMUNE_TO_PC);
