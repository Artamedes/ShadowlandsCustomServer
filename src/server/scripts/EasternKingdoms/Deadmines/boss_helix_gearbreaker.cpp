...
struct boss_helix_gearbreaker : public BossAI
{
    ...
    void JustAppeared() override
    {
        ...
        //DoSummon(NPC_LUMBERING_OAF, LumberingOafSummonPosition, 2s, TEMPSUMMON_CORPSE_TIMED_DESPAWN);
    }
                ...
                case EVENT_SUMMON_HELIX_CREW:
                    for (uint8 i = 0; i < HelixCrewPositionCount; i++)
                    {
                        if (events.IsInPhase(PHASE_1))
                        {
                            ...
                            DoSummon(NPC_HELIX_CREW, HelixSecondCrewPositions[i], 2s, TEMPSUMMON_CORPSE_TIMED_DESPAWN);
                    }
                    break;
    ...
    void PassengerBoarded(Unit* passenger, int8 seatId, bool apply) override
    {
        ...
        if (Creature* helix = passenger->ToCreature())
        {
            if (apply)
            {
                ...
                {
                    helix->RemoveUnitFlag(UNIT_FLAG_UNINTERACTIBLE);
...
class spell_helix_oaf_smash : public SpellScript
{
    ...
    void HandleLaunch(SpellEffIndex effIndex)
    {
        ...
        if (Unit* caster = GetCaster())
        {
            ...
            caster->KnockbackFrom({ x, y, caster->GetPositionZ() }, speedxy, speedz);

            if (Creature* creature = caster->ToCreature())
            {
                creature->SetReactState(REACT_AGGRESSIVE);
                creature->setAttackTimer(BASE_ATTACK, creature->m_unitData->AttackRoundBaseTime[0]);
            }
        }
    }
