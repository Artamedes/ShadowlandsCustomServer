                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SUMMON_VALVES:
                            for (uint8 i = 0; i < 4; i++)
                                DoSummon(NPC_STEAM_VALVE, steamValvePositions[i], 0ms, TEMPSUMMON_MANUAL_DESPAWN);
                            break;
                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_SUMMON_NIGHTMARE_SKITTERLINGS:
                            for (uint8 i = 0; i < 100; i++)
                                me->SummonCreature(NPC_NIGHTMARE_SKITTERLING, nightmareSkitterlingPos[i], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000ms);
                            for (uint8 i = 0; i < 3; i++)
                                me->SummonCreature(NPC_DARKWEB_DEVOURER, DarkwebDevourerPos[i], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000ms);

                            me->SummonCreature(NPC_CHATTERING_HORROR, ChatteringHorrorPos, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000ms);

                            if (Creature* dummy = me->SummonCreature(NPC_GENERAL_PURPOSE_DUMMY_JMF, GeneralPurposeBunnyPos, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5000ms))
            ...
            bool OnReportUse(Player* /*player*/) override
            {
                if (GameObject* cannon = me->ToGameObject())
                {
                    if (_instance->GetData(DATA_BROKEN_DOOR) != DONE)
                    {
                        ...
                        cannon->SetFlag(GO_FLAG_NOT_SELECTABLE);
                    }
                }
                return true;
            }
