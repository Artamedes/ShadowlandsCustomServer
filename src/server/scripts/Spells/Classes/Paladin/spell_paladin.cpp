...
class spell_pal_divine_toll : public SpellScript
{
    ...
    void HandleAfterCast()
    {
        if (auto caster = GetCaster())
        {
            if (auto player = caster->ToPlayer())
            {
                ...
                player->GetAttackableUnitListInRange(enemyList, 30.f);

                if (enemyList.size() > 0)
                {
                    enemyList.sort(Trinity::ObjectDistanceOrderPred(player));
                    if (enemyList.size() > 5)
                        Trinity::Containers::RandomResize(enemyList, 5);
                }
                ...
                switch (player->GetSpecializationId())
                {
                    ...
                    {
                        std::list<Unit*> list;
                        player->GetFriendlyUnitListInRange(list, 30.f, true);
                        list.remove_if([caster](Unit* unit)
                        {
                            if (!unit->ToPlayer())
                                return true;
                        ...
                        if (list.size() > 0)
                        {
                            list.sort(Trinity::HealthPctOrderPred());
                            if (list.size() > 5)
                                Trinity::Containers::RandomResize(list, 5);
                        }

                        for (auto unit : list)
                        {
                            player->CastSpell(unit, DivineToll::HolyShock, castFlags);
                            ++hitTargets;
                        }

                        if (hitTargets < 5)
                        {
                            uint32 diff = 5 - hitTargets;

                            if (enemyList.size() > diff)
                                Trinity::Containers::RandomResize(enemyList, diff);
                            ...
                            for (uint32 i = hitTargets; i < 5; ++i)
                            {
                                auto target = Trinity::Containers::SelectRandomContainerElement(enemyList);
                                player->CastSpell(target, DivineToll::HolyShock, castFlags);
                            }
                    ...
                    case TALENT_SPEC_PALADIN_PROTECTION:
                        ...
                        for (auto unit : enemyList)
                        {
                            player->CastSpell(unit, DivineToll::AvengersShield, castFlags);
                            ++hitTargets;
                        }
                        ...
                        {
                            auto target = Trinity::Containers::SelectRandomContainerElement(enemyList);
                            player->CastSpell(target, DivineToll::AvengersShield, castFlags);
                        }
                    ...
                    case TALENT_SPEC_PALADIN_RETRIBUTION:
                        for (auto unit : enemyList)
                        {
                            player->CastSpell(unit, DivineToll::Judgement, castFlags);
                            ++hitTargets;
                        }
                        ...
                        if (!enemyList.empty())
                        {
                            ...
                            {
                                auto target = Trinity::Containers::SelectRandomContainerElement(enemyList);
                                player->CastSpell(target, DivineToll::Judgement, castFlags);
                            }
                        }
    ...
    void Register() override
    {
        AfterCast += SpellCastFn(spell_pal_divine_toll::HandleAfterCast);
    }
};
