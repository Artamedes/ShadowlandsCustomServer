...
public:
    ...
    bool OnGossipHello(Player* player) override
    {
        ...
        if (auto instance = me->GetInstanceScript())
        {
            if (instance->GetBossState(BossCrystal) != IN_PROGRESS)
            {
                ...
                for (auto const& itr : sDungeonDefenseMgr->m_DefenseTemplates)
                {
                    ...
                    AddGossipItemFor(player, GossipOptionNpc::None, ss.str(), 0, 0, [this, instance, player, diffId, rewardRiftEssence, maxRiftEnergy, starRiftEnergy, maxDefensePoints](std::string_view /*callback*/)
                    {
                        if (instance)
                        {
                            if (instance->GetBossState(BossCrystal) != IN_PROGRESS)
            ...
            {
                AddGossipItemFor(player, GossipOptionNpc::None, "Forfeit", 0, 0, "Are you sure you want to forfeit the event?", 0, false, [this, player, instance](std::string_view /*callback*/)
                {
                    if (instance)
                    {
                        if (instance->GetBossState(BossCrystal) == IN_PROGRESS)
        ...
        AddGossipItemFor(player, GossipOptionNpc::None, "How does this work?", 0, 0, [this, player](std::string_view /*callback*/)
        {
            ClearGossipMenuFor(player);
            AddGossipItemFor(player, GossipOptionNpc::None, "Okay", 0, 0, [this, player](std::string_view /*callback*/)
            {
                OnGossipHello(player);
            });
...
struct npc_darkmaul_citadel_defender_ai : public dungeon_defense_base_ai
{
    ...
    bool OnGossipHello(Player* player) override
    {
        ...
        if (auto instance = player->GetInstanceScript())
        {
            ...
            AddGossipItemFor(player, GossipOptionNpc::None, ss.str(), 0, 0, [this, player](std::string_view /*callback*/)
            {
                OnGossipHello(player);
            });
            ...
            AddGossipItemFor(player, GossipOptionNpc::None, ss.str(), 0, 0, "Are you sure you want to sell this defender?", 0, false, [this, player](std::string_view /*callback*/)
            {
