...
public:
    ...
    struct boss_archdruid_glaidalisAI : public BossAI
    {
        ...
        void JustReachedHome() override
        {
            if (instance)
            {
                if (instance->GetBossState(DATA_ARCHDRUID_GLAIDALIS) != IN_PROGRESS)
                    return;
            ...
            if (phase == PHASE_INTRO)
            {
                ...
                switch (uint32 eventId = events.ExecuteEvent())
                {
                    case EVENT_INTRO_GLAIDALIS_1:
                        for (uint8 i = 0; i < 4; ++i)
                            if (Creature* druid = me->SummonCreature(NPC_DRUIDIC_PRESERVER, SummonPositions[i], TEMPSUMMON_TIMED_DESPAWN, 25000ms))
                                if (Creature* triggerSum = me->SummonCreature(NPC_TRIGGER_PERSERVER, SummonPositions[4 + i], TEMPSUMMON_TIMED_DESPAWN, 25000ms))
                                {
...
class spell_archdruid_glaidalis_grevious_leap_first : public SpellScript
{
    ...
    void HandleHit(SpellEffIndex /*effIndex*/)
    {
        ...
        Unit* target = caster->GetAI()->SelectTarget(SelectTargetMethod::MaxThreat);
        if (!target)
            return;
...
void AddSC_boss_archdruid_glaidalis()
{
    ...
    RegisterSpellScript(spell_archdruid_glaidalis_primal_rage_charge);
}
