...
struct boss_oakheart : public BossAI
{
    ...
    void JustReachedHome() override
    {
        if (instance)
        {
            if (instance->GetBossState(DATA_OAKHEART) != IN_PROGRESS)
                return;
    ...
    {
        if (me->GetDistance(who) < 20.0f && !didIntro && who->IsPlayer())
        {
            didIntro = true;
