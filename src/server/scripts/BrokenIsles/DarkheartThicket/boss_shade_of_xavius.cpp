...
public:
    ...
    struct boss_shade_of_xaviusAI : public BossAI
    {
        ...
        void JustReachedHome() override
        {
            if (instance)
            {
                if (instance->GetBossState(DATA_SHADE_OF_XAVIUS) != IN_PROGRESS)
                    return;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    {
                        events.ScheduleEvent(EVENT_NIGHTMARE_BOLT, urand(17000, 25000));
                        Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 0.0, true);
                        if (!target)
                            break;
...
public:
    ...
    class spell_shade_of_xavius_apocalyptic_nightmare_AuraScript : public AuraScript
    {
        ...
        void HandlePeriodic(AuraEffect const* /*aurEff*/)
        {
            ...
            if (Creature* tempSumm = caster->SummonCreature(WORLD_TRIGGER, caster->GetPositionX() + deltaX - 75, caster->GetPositionY() + deltaY - 75, caster->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 2000ms))
            {
