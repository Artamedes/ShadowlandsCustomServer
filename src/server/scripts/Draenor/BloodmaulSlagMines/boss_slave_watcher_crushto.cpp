        ...
        class boss_SlaveWatcherCrushto : public CreatureScript
        {
            public:
                ...
                struct boss_AI : public BossAI
                {
                    ...
                    void SpellHitTarget(WorldObject* t, SpellInfo const* spellInfo) override
                    {
                        if (t == nullptr || !t->IsUnit())
                            return;
                        ...
                        {
                            instance->instance->DoOnPlayers([this](Player* p_Plr)
                            {
                                /// We check if players are in not behind the boss and if they are in the line.
                                if (me->isInFront(p_Plr) && DistanceFromLine(*me, m_LastEarthCrushStalkerPosition, *p_Plr) < 2.0f && me->GetExactDist2d(p_Plr) < 40.0f)
                                    me->CastSpell(p_Plr, uint32(Spells::EarthCrush3), true);
                            });
                        }
                        ...
                        switch (events.ExecuteEvent())
                        {
                            case uint32(Events::EarthCrush):
                            {
                                if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 40.0f))
                                {
                                    ...
                                    /// We summon the rock.
                                    TempSummon* summon = me->SummonCreature(uint32(MobEntries::EarthCrushStalker), summonPos, TempSummonType::TEMPSUMMON_TIMED_DESPAWN, 4000ms);
                                    if (summon != nullptr)
                                    {
                                        ...
                                        summon->SetUnitFlag(UnitFlags(UNIT_FLAG_UNINTERACTIBLE | UNIT_FLAG_NON_ATTACKABLE));
                                    }
                                }
