        ...
        public:
            ...
            struct mob_AI : public ScriptedAI
            {
                ...
                void Reset() override
                {
                    ...
                    me->SetUnitFlag(UnitFlags(UNIT_FLAG_UNK_6 | UNIT_FLAG_CAN_SWIM | UNIT_FLAG_UNINTERACTIBLE));
                }
                ...
                void SetData(uint32 type, uint32) override
                {
                    if (type == uint32(Data::SpawnSlagna))
                    {
                        ...
                        me->RemoveUnitFlag(UnitFlags(UNIT_FLAG_UNK_6 | UNIT_FLAG_CAN_SWIM | UNIT_FLAG_UNINTERACTIBLE));
    ...
    class mob_CapturedMiner : public CreatureScript
    {
        public:
            ...
            struct mob_AI : public ScriptedAI
            {
                mob_AI(Creature* creature) : ScriptedAI(creature)
                {
                    me->ReplaceAllUnitFlags(UnitFlags(0));
                }
                    ...
                    switch (eventId)
                    {
                        ...
                        case (uint32)Events::EVENT_SLAVE_NO:
                        {
                            if (Creature* enslavedMiner = me->GetMap()->GetCreature(slaveGuid))
                            {
                                ...
                                enslavedMiner->GetMotionMaster()->MoveKnockbackFrom(*me, 10.0f, 10.0f);
                                if (enslavedMiner->AI())
                                    enslavedMiner->AI()->Talk((uint32)Texts::TEXT_SLAVE_NO);
                                enslavedMiner->DespawnOrUnsummon(1000ms);
                            }
                            return true;
                        }
                ...
                void UpdateAI(const uint32 diff) override
                {
                    ...
                    switch (events.ExecuteEvent())
                    {
                        case uint32(Events::ChainGrip):
                            if (Unit* l_Plr = SelectTarget(SelectTargetMethod::MaxDistance, 0, 5.0f))
                            ...
                            if (Player* l_Plr = ObjectAccessor::FindPlayer(m_TargetGUID))
                            {
                                ...
                                for (Player* player : players)
                                    if (me->GetExactDist2d(player) <= 5.0f)
                                        player->GetMotionMaster()->MoveKnockbackFrom(*me, 2.0f, 2.0f);
                            }
                            break;
                    }
                }
                ...
                void UpdateAI(const uint32 diff) override
                {
                    ...
                    switch (events.ExecuteEvent())
                    {
                        ...
                        case uint32(Events::LumberingLeap):
                            if (Unit* l_Plr = SelectTarget(SelectTargetMethod::MaxDistance, 0, 8.0f))
                            {
        ...
        public:
            ...
            struct mob_bloodmaul_exploding_flamesAI : public ScriptedAI
            {
                ...
                void Reset() override
                {
                    ...
                    me->SetUnitFlag(UnitFlags(UNIT_FLAG_REMOVE_CLIENT_CONTROL | UNIT_FLAG_UNINTERACTIBLE | UNIT_FLAG_NON_ATTACKABLE));
                }
