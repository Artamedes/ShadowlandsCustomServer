...
inline Player* SelectRandomPlayerIncludedTank(Unit* me, float range = 0.0f, bool checkLoS = true)
{
    Map* map = me->GetMap();
    if (!map->IsDungeon())
        return nullptr;
    ...
    {
        if (!i->GetSource()->IsGameMaster() && (me->IsWithinLOSInMap(i->GetSource()) || !checkLoS)
...
inline void ApplyOnEveryPlayer(Unit* me, std::function<void(Unit*, Player*)> function)
{
    Map* map = me->GetMap();
    if (!map->IsDungeon())
        return;
...
inline Player* SelectRandomPlayerExcludedTank(Unit* me, float range = 0.0f, bool checkLoS = true)
{
    ...
    for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
    {
        if (!i->GetSource()->IsGameMaster() && (me->IsWithinLOSInMap(i->GetSource()) || !checkLoS) && me->GetVictim() != i->GetSource() &&
...
inline std::list<Unit*> SelectNearestCreatureListWithEntry(Unit* me, uint32 entry, float range = 0.0f)
{
...
inline Unit* SelectNearestFriendExcluededMe(Unit* me, float range = 0.0f, bool checkLoS = true)
{
    ...
    {
        if (l_Unit && (me->IsWithinLOSInMap(l_Unit) || !checkLoS) &&
            me->IsWithinDistInMap(l_Unit, range) && l_Unit->IsAlive() && l_Unit->GetGUID() != me->GetGUID())
        {
            return l_Unit;
        }
    }
...
enum RandomSpells
{
    ...
    CLOACK = 165848,
    INTRO_NARRATOR = 163922, ///< Casted by player on zone in first time
    TWISTER_DNT = 178617,
