...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_UPD_CREATURE_MOVEMENT_TYPE, "UPDATE creature SET MovementType = ? WHERE guid = ?", CONNECTION_BOTH);
    ...
    PrepareStatement(WORLD_INS_CREATURE_ADDON, "INSERT INTO creature_addon(guid, path_id, auras) VALUES (?, ?, ?)", CONNECTION_BOTH);
    PrepareStatement(WORLD_UPD_CREATURE_ADDON_PATH, "UPDATE creature_addon SET path_id = ?, auras = ? WHERE guid = ?", CONNECTION_BOTH);
    PrepareStatement(WORLD_DEL_CREATURE_ADDON, "DELETE FROM creature_addon WHERE guid = ?", CONNECTION_BOTH);
    PrepareStatement(WORLD_SEL_CREATURE_ADDON_BY_GUID, "SELECT guid, auras, path_id FROM creature_addon WHERE guid = ?", CONNECTION_SYNCH);
