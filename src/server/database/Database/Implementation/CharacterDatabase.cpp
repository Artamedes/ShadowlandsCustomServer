...
#define SelectItemInstanceContent "ii.guid, ii.itemEntry, ii.creatorGuid, ii.giftCreatorGuid, ii.count, ii.duration, ii.charges, ii.flags, ii.enchantments, ii.randomBonusListId, " \
    ...
    PrepareStatement(CHAR_SEL_CHARACTER, "SELECT c.guid, account, name, race, class, gender, level, xp, money, inventorySlots, bankSlots, restState, playerFlags, playerFlagsEx, "
                     ...
                     "health, power1, power2, power3, power4, power5, power6, power7, power8, power9, power10, instance_id, activeTalentGroup, lootSpecId, exploredZones, knownTitles, actionBars, raidDifficulty, legacyRaidDifficulty, fishingSteps, "
    ...
    PrepareStatement(CHAR_SEL_ACCOUNT_ACHIEVEMENTS, "SELECT achievement, date, first_guid FROM account_achievement where account = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_CHAR_ACC_ACHIEVEMENT, "INSERT INTO account_achievement (account, achievement, first_guid, date) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_CHAR_ACC_ACHIEVEMENT_BY_ACHIEVEMENT, "DELETE FROM account_achievement WHERE achievement = ? AND account = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_CHAR_ACC_ACHIEVEMENT, "UPDATE account_achievement SET achievement = ? where achievement = ? AND account = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_CHALLENGE_KEY, "REPLACE INTO challenge_key (ItemID, ID, Level, Affix, Affix1, Affix2, Affix3, KeyIsCharded, timeReset, InstanceID, guid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
